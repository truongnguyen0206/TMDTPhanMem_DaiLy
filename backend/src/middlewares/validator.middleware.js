...
const validateExcelUpload = (req, res, next) => {
    ...
    if (!validTypes.includes(fileType)) {
        return res.status(400).json({ error: 'Invalid file type. Please upload Excel file' });
    }

    next();
};

// Validate withdrawal request data <--- BỔ SUNG HÀM NÀY
const validateWithdrawalRequest = (req, res, next) => {
    const { amount } = req.body;
    const errors = [];

    // Validate amount (required, number, positive, min 1,000,000)
    if (!amount || isNaN(amount) || parseFloat(amount) <= 0) {
        errors.push('Số tiền rút là bắt buộc và phải là số dương');
    } else if (parseFloat(amount) < 1000000) { 
         errors.push('Số tiền rút tối thiểu là 1,000,000 VND');
    }
    ...
    // Convert string number to proper type
    req.body.amount = parseFloat(amount);

    next();
};

// SỬA LỖI: Hợp nhất tất cả các hàm cần export vào MỘT module.exports duy nhất
module.exports = {
    validateId,
